...
@Component({
  selector: 'app-admin-modifier-offre',
  templateUrl: './admin-modifier-offre.component.html',
  styleUrls: ['./admin-modifier-offre.component.css']
})
export class AdminModifierOffreComponent implements OnInit {
  ...
  updateService(): void {
    this.serviceModelService.updateService(this.service).subscribe(() => {
      this.router.navigate(['/AdminOffre']); // Navigate back to service list after modification
    }, error => {
